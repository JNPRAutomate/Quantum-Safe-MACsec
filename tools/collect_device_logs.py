...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        help=(
            "SSH private key; defaults to the etsi_user key generated by the "
            "first deployment"
        ),
    )
    parser.add_argument(
...
def discover_identity_file(
    ...
    if explicit is not None:
        ...
        if not candidate.is_file():
            raise RuntimeError("Identity file not found: %s" % candidate)
        return candidate

    home = home or Path.home()
    candidates = [
        home / ".ssh" / ("qkd_%s_qkd_id_ed25519" % user),
        home / ".qkd" / "script_user_keys" / user / "qkd_id_ed25519",
    ]
    for candidate in candidates:
        if candidate.is_file():
            return candidate
    raise RuntimeError(
        "QKD script-user identity not found. Expected %s or %s; "
        "run the script-user bootstrap or pass --identity-file."
...
def main() -> int:
    ...
    try:
        ...
        identity_file = discover_identity_file(user, args.identity_file)
