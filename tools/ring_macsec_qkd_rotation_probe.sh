...
# - Capture MACsec/MKA/LACP/ISIS state and QKD on-box timeline markers.
# - Produce a single test report log with failure counters.
...
OUT="/var/tmp/ring_macsec_qkd_rotation_probe_${TEST_TS}.log"
EVENTLOG="/var/tmp/ring_macsec_qkd_events_${TEST_TS}.log"
...
# Space-separated interface list used for qkd_onbox status.
QKD_IFACES="${QKD_IFACES:-et-2/0/4 et-2/0/2}"

# Space-separated pairs CA:KEYCHAIN used for config checks.
QKD_CAS="${QKD_CAS:-CA1:QKD_CA1 CA9:QKD_CA9}"

# New runtime path first, legacy fallback second.
QKD_LOG_GLOBS="/var/home/admin/logs/qkd_debug*.log /var/tmp/qkd_debug*.log"
...
capture_qkd_timeline()
{
    ...
    for pattern in $QKD_LOG_GLOBS; do
        run_shell "qkd scheduled/pending/promoted timeline ($pattern)" \
            "for f in $pattern; do [ -f \"\$f\" ] || continue; echo \"### \$f\"; grep -E 'KEYCHAIN ROTATION START|INSTALL-KEY SCHEDULE|STATE SAVED|pending_key_id|next_start_time|MKA KEY CONFIRMED|PENDING KEY PROMOTED|PENDING_KEY_NOT_CONFIRMED|KEYCHAIN ROTATION DONE|KEYCHAIN INSTALL FAIL|INSTALL-KEY ABORTED|MACSEC NOT INUSE|UNKNOWN_CAK|DOT1XD_MACSEC_SC_UNKNOWN_CAK_ERR' \"\$f\" | tail -120; done"
    done
}
...
capture_recent_events()
{
    ...
    for pattern in $QKD_LOG_GLOBS; do
        run_shell "qkd debug recent errors ($pattern)" \
            "for f in $pattern; do [ -f \"\$f\" ] || continue; echo \"### \$f\"; grep -E 'ERROR|FAIL|FAILED|KEYCHAIN|MKA|PENDING|PROMOTED|ROTATION|BOOTSTRAP|INSTALL-KEY|MACSEC|SSH|DEC|ENC|KME' \"\$f\" | tail -80; done"

        run_shell "qkd debug recent tail ($pattern)" \
            "for f in $pattern; do [ -f \"\$f\" ] || continue; echo \"### \$f\"; tail -40 \"\$f\"; done"
    done
}
...
summary()
{
    ...
    run_shell "Count MACsec not-inuse / commit / keychain failures" \
    ...
    run_shell "Count state save failures" \
        "grep -E 'STATE SAVE ERROR|Operation not permitted' '$OUT' | wc -l"
    ...
    section "EXPECTED HEALTHY BEHAVIOR"
...
section "Ring MACsec/QKD/MKA scheduled rotation probe"
