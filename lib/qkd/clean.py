...
def clean_device(
    ...
    try:
        ...
        # Remove QKD SSH keys from the script_user home dir.
        # These are generated by the orchestrator deploy and are NOT removed
        # by Junos when the user is deleted from config. Without this, a
        # subsequent deploy would find stale valid keys and skip regen,
        # leaving the device in an inconsistent state.
        ssh_dir = f"{ssh_home_base}/{script_user}/.ssh"
        ssh_key_name = str(QKD.get("SSH_KEY_NAME", "qkd_id_ed25519"))
        peer_key_name = str(QKD.get("PEER_CMD_SSH_KEY_NAME", "qkd_peer_cmd_ed25519"))
        for key_name in (ssh_key_name, peer_key_name):
            file_cleanup_parts.append(
                f"rm -f {ssh_dir}/{key_name} {ssh_dir}/{key_name}.pub "
                f"{ssh_dir}/{key_name}.next {ssh_dir}/{key_name}.next.pub"
            )
