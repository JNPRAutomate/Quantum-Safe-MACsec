...
def repair_script_user_ssh_state(device):
    ...
    cmd = (
        ...
        f"chown {script_user} {ssh_dir} {auth_path} >/dev/null 2>&1; "
        f"if [ -f {key_path} ]; then chown {script_user} {key_path} >/dev/null 2>&1; fi; "
        f"if [ -f {pub_path} ]; then chown {script_user} {pub_path} >/dev/null 2>&1; fi; "
        f"ls -ld {ssh_dir}; "
        f"ls -l {auth_path}; "
...
def check_script_user_home_simple(device):
    ...
    cmd = (
        ...
        f"mkdir -p {ssh_dir}; "
        f"chmod 700 {ssh_dir}; "
        f"chown {script_user} {ssh_dir} >/dev/null 2>&1; "
        f"ls -ld {ssh_dir}"
    )
...
def check_script_user_ssh_identity(device):
    ...
    cmd = (
        ...
        f"chown {script_user} {ssh_dir} {key_path} {pub_path} >/dev/null 2>&1; "
