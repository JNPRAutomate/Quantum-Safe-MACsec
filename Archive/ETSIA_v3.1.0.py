...
The script is supposed to be kept at /var/db/scripts/event/onbox.py
...
KEYID_JSON_FILENAME = '/home/telcocloud/Documents/juniper_ps/etsi-gs-qkd-014-referenceimplementation-main/{}_last_key_test.json'
# KEYID_JSON_FILENAME = '/home/administrator/{}_last_key_test.json'
# CERTS_DIR = '/var/tmp/acx1/'
# CERTS_DIR = '/home/telcocloud/Documents/juniper_ps/etsi-gs-qkd-014-referenceimplementation-main/certs'


prof = Profile.Profile(file="/home/telcocloud/Documents/juniper_ps/etsi-gs-qkd-014-referenceimplementation-main/scaler.prof", verbose=True, enabled=True, mode="w+")
...
def fetch_kme_key(session, local_name, log, remote_mnmgt_add, kme_url, key_id=None, additional_slave_SAE_IDs=None):
    ...
    try:
        if key_id:
            ...
            # response = session.get(f"{kme_url}enc_keys", verify=CA_CERT, cert=CLIENT_CERT)
...
def check_and_apply_initial_config(dev, targets_dict, log):
    ...
    if onbox:
        initial_macsec_commands.extend([
            ...
            f"set event-options policy qkd then event-script onbox.py",
    ...
    if targets_dict["qkd_roles"]['master'] == local_name:
        ...
        while True:
            if retries > 60:
                ...
                if last_key_dict.get(local_name) == new_key_dict[local_name]:
                    retries += 1
                    ...
                    break
        log.info(f'local_name: {local_name} Previous keyID: ' + last_key_dict[local_name])
        print(f'local_name: {local_name} Previous keyID: ' + last_key_dict[local_name])
        r = fetch_kme_key(session, local_name, log, remote_mnmgt_add, kme_url, key_id=new_key_dict[local_name])
        if r is not None:
            print(r)
            log.info(f'local_name: {local_name} KME: [POST] Get Key with key_ID: ' + new_key_dict[local_name])
            key = r['keys'][0]
        else:
            return
