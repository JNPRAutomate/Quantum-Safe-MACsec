...
class QKDOrchestrator:
    ...
    def self._set_mode_if_needed(self, path_obj: Path, target_mode: int) -> Tuple[bool, str]:
        """
...
# ============================
    ...
    def self._active_slot_from_state(self, state):
        active_key_id = state.get("active_key_id")
        if not active_key_id:
            return None
    ...
    def _assign_slots_for_entries(self, state, entries):
        ...
        active_slot = self._active_slot_from_state(state)
        used = set()
    ...
    def self._status_payload_for_link(self, link):
    ...
    def _export_peer_status_snapshot(self, link, state=None):
        ...
        if state is None:
            payload = self._status_payload_for_link(link)
        else:
            payload = dict(state)
    ...
    def _run_slave_status(self, iface):
        if not iface:
            payload = []
            for link in self._managed_links():
                state = self._status_payload_for_link(link)
